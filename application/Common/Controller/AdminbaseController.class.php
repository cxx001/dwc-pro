...
class AdminbaseController extends AppframeController
{
    ...
    protected $_curlHost = '127.0.0.1:83';
    private $_defaultHost = '127.0.0.1:83';
    ...
    function _initialize()
    {
        ...
        if (!empty($session_admin_id)) {
            if ($session_admin_id != 1) {
                switch (session('name')) {
                    default: {
                        $this->_curlHost = $this->_defaultHost;
                    }
                        break;
                }
            }
            if ($session_admin_id == 1 && !F('admin_host')) {
                F('admin_host', $this->_defaultHost);
                F('admin_pre', 'md');
                F('admin_name', '电玩城');
            }
            $users_obj = M("Users");
    ...
    {
        F('qunhei_url', $this->_qunheiHost);
        F('baidu_url', $this->_baiduHost);
        F('hangzhou_url', $this->_hangzhouHost);
        F('jiuyaowan_url', $this->_jiuyaowanHost);
        F('yxd_url', $this->_yxdHost);
        F('cs_url', $this->_defaultHost);
        F('qunhei_pre', 'qunhei');
        F('baidu_pre', 'baidu');
        F('hangzhou_pre', 'my2016');
        F('jiuyaowan_pre', 'jiuyaowan');
        F('yxd_pre', 'md');
        F('cs_pre', 'my2016');
    }
